-- The previous fix renamed the function's locals to v_workspace_id /
-- v_workspace_slug, but the `RETURNS TABLE(workspace_id, workspace_slug)`
-- declaration still creates implicit OUT variables with those names. They
-- remain in scope and collide with the real column `workspace_id` in
-- `INSERT INTO workspace_members (workspace_id, ...)`,
-- `INSERT INTO workspace_usage_wallet (workspace_id, ...)`, and
-- `ON CONFLICT (workspace_id)` — which is what the "column reference
-- workspace_id is ambiguous" error was actually about.
--
-- The targeted fix is the `#variable_conflict use_column` directive: when
-- an identifier could mean either a plpgsql variable or a column, prefer
-- the column. The OUT params are still set explicitly via the assignments
-- at the end of the function, so this is safe.
