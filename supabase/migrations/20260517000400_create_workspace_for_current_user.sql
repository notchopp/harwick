...
begin
  ...
  while exists (select 1 from public.workspaces where slug = case when v_suffix = 0 then v_slug else v_slug || '-' || v_suffix::text end) loop
    v_suffix := v_suffix + 1;
  end loop;
  if v_suffix > 0 then
  ...
  returning id, slug into workspace_id, workspace_slug;
