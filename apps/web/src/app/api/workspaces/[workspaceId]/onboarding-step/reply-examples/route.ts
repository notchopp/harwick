...
export async function POST(
  ...
  const body = CaptureReplyExamplesInputSchema.safeParse(await request.json().catch(() => null));
  if (!body.success) {
  ...
  try {
    ...
    const saved = await persistReplyExamples(supabase, workspaceId, body.data);
    const state = await markOnboardingBeatComplete(supabase, workspaceId, "reply_examples");
    ...
    return NextResponse.json(
      {
        error: "reply_examples_persist_failed",
