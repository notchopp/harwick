...
async function authorizeWorkspaceWrite(request: NextRequest, workspaceId: string) {
  ...
  if (membership === null || !listingWriteAllowedRoles.has(membership.role)) {
    return null;
  }
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const membership = await authorizeWorkspaceWrite(request, workspaceId);
  if (membership === null) {
