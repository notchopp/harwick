...
| Harwick AI runtime | partial | typed runtime, local/OpenAI adapters, tool contracts, tool registry prompt construction, durable subagent task dispatch, subagent execution cron, scheduled Harwick loop definitions/runs, settings UI for loop creation/pause/resume, draft loop output, approval-first agent-loop plans, operator approval for safe internal loop execution, hydrated conversation automation/qualification runtime state, member-scoped assigned-agent policy resolution in the executor, production executor coverage for auto-send, approval-required tools, blocked human takeover, and plan-capacity pause, persisted turn/tool execution outputs, worker-side outbound message mirroring, automation policy, canonical Meta send tool policy defaults/backfill, and tool executor exist | Finish provider-backed validation before autonomous loop external writes and keep worker/provider auto-send coverage aligned with every new channel. |
...
| Public listings | partial | public workspace listing surface loads persisted `listing_facts`, dedicated per-listing public routes exist, listing pages embed Harwick as the primary buyer interaction, no longer falls back to demo inventory or fake phone numbers, public inquiry route creates/updates leads and lead events, listing-scoped showing requests create approval tasks, listing-scoped open-house registrations create attendee tasks, and public chat now feeds Harwick listing area intel, returning visitor context, team context, callback tasks, CMA requests, and showing-window approval tasks through the same lead-promotion path | Template variants, live enrichment job scheduling, and reminder delivery. |
| Follow Up Boss | partial | sync, back-sync, logs, webhook subscription foundation, server-only key save/test routes, pasted-key validation before encrypted save, integrations-page saved-key test UX, audited conflict replay/ignore APIs, home work-queue conflict replay/ignore actions, and async `fub_sync` worker retry handling exist | Worker path and public listing push still need staging provider validation against real Follow Up Boss credentials. |
...
| Voice operations | partial | Retell provisioning/context/tools/handoffs exist, provisioning can create the Realty Ops conversation flow directly without a template asset, owners/launch managers can claim or resync a workspace Retell number from onboarding activation and Settings, Settings shows the current workspace voice number/status/service areas/transfer target, Retell webhooks strip raw transcripts, sanitized post-call analysis now creates an idempotent voice handoff queue item when the in-call handoff tool was not already used, Retell call handoffs have a database call-id uniqueness guard, and voice queue callback actions create scheduled call-back tasks with phone/call context while avoiding duplicate callback-task creation | Production webhook validation and live Retell callback workflow smoke. |
...
| Reliability hardening | partial | job model, provider errors, release gates, audit logs, route rate limits on public/provider surfaces, staging/prod runtime readiness checks, launch rollback notes, idempotency coverage pass, staging/production `.env.local` fallback disabled, top-level app shell/settings no longer render fake profile/sidebar counts, local/system-backed app typography with no remote Google Fonts build dependency, and staging-safe public provider smoke fixture exist; Harwick FUB tool sync now upserts a stable `fub_sync:{leadId}` qualified-only worker job | Run provider smoke with `LAUNCH_PROVIDER_SMOKE_REQUIRED=true` against deployed staging before production launch; install/validate Sentry only after a Next 16-compatible SDK path is available. |
| Launch test harness | partial | `npm run launch:check` runs launch readiness fixtures, staging provider smoke fixture, release gate, production build, and remote Supabase RLS verification in one command; latest local `npm test` passes 166 test files / 776 tests; latest `npm run launch:check` passes launch fixtures, release gate, production build, and remote RLS verification; `npm run test:launch-readiness` proves inbound event -> AI -> queue/send -> lead -> route -> FUB sync -> Harwick loop approval -> safe subagent queueing without real provider writes and validates staging/prod runtime config gates; `npm run test:staging-provider-smoke` verifies the smoke harness and can hit deployed readiness, Meta challenge, and Retell signed no-op webhook paths when a staging URL is configured, including Vercel deployment-protection bypass when `LAUNCH_PROVIDER_SMOKE_BYPASS_SECRET` is set and deployed readiness missing-key reporting when the target is blocked; `npm run launch:env:audit` verifies deployed Vercel env names without printing secrets and can be required from `npm run launch:check` with `LAUNCH_ENV_AUDIT_REQUIRED=true`; `npm run launch:env:sources` reports local exact/alias env source coverage without printing values and currently identifies the remaining unsourced names as `RETELL_VOICE_ID`, annual/team/brokerage Stripe price IDs, and `GOOGLE_CALENDAR_OAUTH_REDIRECT_URI`; Retell template flow is now optional because provisioning can create the Realty Ops flow directly | Deployed smoke against `https://harwick.lol` still requires deployed production/staging env completion, including `RETELL_VOICE_ID`; run the provider smoke fixture and env audit against a deployed staging app with real staging env before production launch. |
...
- ✅ dedicated per-listing public routes with embedded Harwick chat
- open-house CTA path
- ✅ listing-aware AI context from internal facts, cached area intel, returning visitor state, and workspace team routing
...
- ✅ async `fub_sync` worker with credential lookup, listing context hydration, retry/backoff, and no fake contact-field fallback
...
- Policy persistence alignment: Supabase policy defaults and existing rows are backfilled for the runtime's canonical `send_meta_message` tool and safe internal `dispatch_subagent` allowlist entries, with launch migration coverage.
- Runtime state alignment: Harwick turn generation and the agentic executor hydrate conversation automation mode plus lead qualification into `HarwickAiRuntimeInput.state`, and thread the same automation mode into Meta tool execution.
- Member policy alignment: the AI-native executor resolves effective automation policy with the lead's assigned agent/member id when present, instead of only applying workspace/conversation scopes.
- Launch harness reliability: the app shell now uses local/system-backed typography instead of `next/font/google`, so production builds no longer fail when `fonts.googleapis.com` is unavailable; `npm run launch:check` is green with remote RLS verification.
- Executor policy coverage: the production Harwick agentic executor now has focused tests proving approval-required tools stay queued with persisted execution output, human-takeover turns persist as blocked without outbound sends, and plan-capacity pauses happen before model spend.
- Conversation control parity: Leads detail sheets and the mobile Home queue drawer now use the same compact `LeadActionToolbar` as Conversations, so pause/resume/takeover/release actions call the audited conversation automation API instead of local-only UI state.
