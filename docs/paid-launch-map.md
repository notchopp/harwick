...
| Usage metering | partial | usage event tables, usage wallet, usage summary aggregation, plan gate service, current-period usage recorder, wallet-backed Harwick/Retell/memory usage events, and settings Plan & Usage surface exist | Finish pre-run overage debit/gating and exercise upgrade prompts against real launch usage. |
