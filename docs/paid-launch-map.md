...
| Harwick workspace memory | partial | `workspace_memory_documents` table, typed contract, repository, `/api/agent-runtime/workspace-memory` distillation worker, and Harwick runtime prompt retrieval exist for repeated routing override patterns | Embed memories semantically and broaden distillation beyond routing overrides. |
...
Current AI-native completion estimate: **47%**.
...
- Semantic workspace memory retrieval with embeddings.
