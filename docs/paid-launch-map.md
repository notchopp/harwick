...
| Billing/plans | done | subscription tables, plan schemas, usage events, plan capability contracts, server-only Stripe checkout and customer portal clients/APIs, Stripe env validation, signature-verified Stripe webhook route, idempotent billing webhook ledger, subscription reconciliation, and owner/admin billing controls in settings exist | Keep Stripe endpoint configured in staging/prod and continue metering real usage events. |
| Usage metering | partial | usage event tables, usage summary aggregation, plan gate service, current-period usage recorder, and durable usage recording from lead events, social replies, Harwick AI turns, sent nurture/open-house reminder receipts, and Retell billable voice minutes exist | Keep overage handling and upgrade prompts exercised against real launch usage. |
