...
| Harwick proactive insight feed | partial | `harwick_work_items` exists; `/api/agent-runtime/insights` cron producer surfaces ambiguous inbound, unassigned priority leads, and dormant active leads; `/home` shows member/role-filtered insights with seen/dismiss actions | Add model-distilled workspace pattern insights and operator feedback labels. |
| Harwick workspace memory | partial | `workspace_memory_documents` table, typed contract, repository, and `/api/agent-runtime/workspace-memory` distillation worker exist for repeated routing override patterns | Embed memories, retrieve them in the Harwick runtime prompt, and broaden distillation beyond routing overrides. |
...
Current AI-native completion estimate: **44%**.
...
- Workspace memory retrieval inside the Harwick runtime prompt.
